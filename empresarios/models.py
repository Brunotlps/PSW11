...
class Empresas(models.Model):
    ...
    def status(self):
        if date.today() > self.data_final_captacao:
            return mark_safe('<span class="badge rounded-pill text-bg-success">Captação Finalizada</span>')
        return mark_safe('<span class="badge rounded-pill text-bg-primary">Captação em andamento</span>')
