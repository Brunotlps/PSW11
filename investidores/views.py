...
def ver_empresa(request, id):
    ...
    # TODO: Listar as métricas dinâmicamente
...
def realizar_proposta(request, id):
    ...
    if total + float(percentual) > empresa.percentual_equity:
        messages.add_message(request, constants.WARNING, 'Percentual solicitado ultrapassa o valor disponível')
        return redirect(f'/investidores/ver_empresa/{id}')

    valuation = (100 * int(valor)) / int(percentual) 
    if valuation < (int(empresa.valuation / 2)):
        messages.add_message(request, constants.WARNING, f'Valuation proposto negado, valor mínimo aceito é de R${empresa.valuation}')
        return redirect(f'/investidores/ver_empresa/{id}')
